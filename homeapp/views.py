...
def nuevo_proyecto(request):
    if request.method == 'POST':
        try:
            nombre = request.POST['title']
            descripcion = request.POST['description']
            fecha_inicio = datetime.strptime(request.POST['start-date'], '%Y-%m-%d').date()
            fecha_final = datetime.strptime(request.POST['end-date'], '%Y-%m-%d').date()
            presupuesto = float(request.POST['presupuesto'])
            constructora = request.user
            ...
            proyecto.save()
            return redirect('nuevo-proyecto')  # Ajusta la URL de redirección según sea necesario

        except Exception as e:
            print(f"Error al crear el proyecto: {e}")  # Imprime el error en la consola para depuración
...
def proyectos_info(request):
    ...
    return render(request, 'proyectos.html', {'proyectos': proyectos, 'usuarios': proyecto_usuarios, 'usuario_actual': usuario_actual,'show_link': show_link}) #'tareas' : tareas , 'roles': roles,'show_link': show_link})
